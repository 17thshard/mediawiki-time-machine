...
class SpecialTimeMachine extends SpecialPage {
	...
	public function execute( $parser ) {
		...
		$currentlyTimeTraveling = TimeMachineUtils::getTimeTravelTarget( $request ) !== false;
		$date = $request->getCookie( TimeMachineUtils::COOKIE, null, date( 'Y-m-d' ) );
		...
		if ( $request->wasPosted() ) {
			...
			if ( $currentlyTimeTraveling ) {
				$response->setCookie( TimeMachineUtils::COOKIE, $date );
			} else {
				$response->clearCookie( TimeMachineUtils::COOKIE );
			}
