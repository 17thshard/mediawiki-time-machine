...
class TimeMachineUtils {
	...
	public static function getTimeTravelTarget( WebRequest $request ) {
		$date = $request->getCookie( TimeMachineUtils::COOKIE );
	...
	public static function timeTravelRevision( Title $title, string $timestamp ) {
		...
		return $cache->getWithSetCallback(
			$cache->makeGlobalKey( 'timemachine', $articleID, $timestamp ),
			$cache::TTL_DAY,
	...
	public static function findMoveSourceAfter( Title $referenceTitle, string $timestamp ) {
		...
		return $cache->getWithSetCallback(
			$cache->makeGlobalKey(
				...
				$timestamp
			), $cache::TTL_DAY, function () use ( $referenceTitle, $timestamp ) {
			$dbr = wfGetDB( DB_REPLICA );
			$result = $dbr->selectField(
				'timemachine_title_history', 'tm_page_id', [
